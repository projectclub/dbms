...
Class Faculty{
	...
	public function getTeachingCourse($faculty_id, $year) {
		$result = $this->conn->query("SELECT `teaches`.`course_faculty_year_id`, `teaches`.`course_id`, `course_component`.`type`, `course`.`title`,`year`, `course`.`semester` FROM `teaches` JOIN `course_component` ON `course_component`.`course_faculty_year_id` = `teaches`.`course_faculty_year_id` JOIN `course` ON `course`.`course_id` = `teaches`.`course_id` WHERE `teaches`.`faculty_id` = ". $faculty_id. " and `year` = ".$year);
		$data=array();
		while($rs = $result->fetch_array(MYSQLI_ASSOC)) {
...
/*		  $data[$rs["course_id"]]["type"] = [];
	...
	public function getCourseEnrollment($course_faculty_year_id) {
		/*("SELECT `rollno`, `enrollment`.`course_faculty_year_id`, `course`.`title`, `course`.`semester`, `faculty`.`faculty_id`, `faculty`.`faculty_name` FROM `enrollment` JOIN `teaches` ON  `enrollment`.`course_faculty_year_id`= `teaches`.`course_faculty_year_id` JOIN `course` ON `teaches`.`course_id`= `course`.`course_id` JOIN `faculty` ON `teaches`.`faculty_id`= `faculty`.`faculty_id` WHERE `enrollment`.`course_faculty_year_id` = ". $course_faculty_year_id);*/

		$result = $this->conn->query("SELECT `enrollment`.`rollno`,`student`.`name`, `student`.`gender` FROM `enrollment` JOIN `teaches` ON  `enrollment`.`course_faculty_year_id`= `teaches`.`course_faculty_year_id` JOIN `student` ON `student`.`rollno`= `enrollment`.`rollno` WHERE `enrollment`.`course_faculty_year_id` = ". $course_faculty_year_id);
		$data=array();
